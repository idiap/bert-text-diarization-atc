...
"""
    Script with some utils functions to train and evaluate text-based diarization model. 
...
class ATCDataset_diarization(torch.utils.data.Dataset):
...
def read_atc_diarization_data(file_path, max_seq_len=60):
...
def get_index_value(raw_pred):
    """get the index and confidence given the outputs. We use a SoftMax Layer"""

    # We need first a SoftMax Layer
    softmax_l = torch.nn.Softmax(dim=1)
    pred_conf = softmax_l(torch.tensor(raw_pred))
    scores, indices = pred_conf.max(1)
...
def compute_metrics_sklearn(p):
    ...
    # metrics related to Diarization
