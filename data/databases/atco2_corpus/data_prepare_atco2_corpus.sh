...
# different folder
data=$EXP_FOLDER/atco2_corpus; mkdir -p $data/prep
...
for wav in $(cat $data/list_of_wav); do
    ...
    # echo "$key $sox -t wav $wav -t wav -r$sampling_rate -b16 - remix - |"
    echo "$key $wav"
done >$data/prep/wav.scp
...
cut -d';' -f1,8- $data/prep/transcripts2_2.csv | sed 's/;/ /' > $data/prep/transcripts2_3.csv 
...
python3 data/utils/normalizer/final_normalization.py \
  ...
  --input $data/prep/transcripts2_3.csv \
  --output $data/prep/text_final
...
paste -d ' ' <(awk -F';' '{ print $2, "A", $5, $6, $7; }' $data/prep/transcripts3.csv) \
...
# text,
awk -F';' '{ print $1, $8; }' <$data/prep/transcripts3.csv >$data/prep/text

# segments,
awk -F';' '{ print $1, $2, $6, $7; }' <$data/prep/transcripts3.csv >$data/prep/segments

# utt2spk, 
awk -F';' '{ print $1, $2"-"$3; }' <$data/prep/transcripts3.csv | LC_ALL=C sort >$data/prep/utt2spk

# reco2file_and_channel
awk -F';' '{ print $2, $2, "A"; }' <$data/prep/transcripts3.csv >$data/prep/reco2file_and_channel
...
# utt2nertags
cut -d';' -f1,9 <$data/prep/transcripts3.csv |tr ';' ' ' >$data/prep/utt2nertags

# utt2callsign
cut -d';' -f1,10 $data/prep/transcripts3.csv | tr ';' ' ' >$data/prep/utt2callsign

# copy final versions of the data into the main folder
echo "copying the final version of the data into $data/"
cp $data/prep/{utt2spk,text,stm,reco2file_and_channel,segments,wav.scp} $data/
...
cp $data/prep/{utt2waypoint_list,utt2callsign_list,utt2airport,utt2speaker_callsign,utt2nertags,utt2callsign} $data/
